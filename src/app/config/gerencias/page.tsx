...
export default function GerenciasPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [gerRes, orgRes] = await Promise.all([
  ...
  const handleSave = async () => {
    ...
    const url = isEditing ? `/api/config/gerencias/${isEditing}` : "/api/config/gerencias";
    ...
    <Shell>
      <div className="page-header animate-fade-in">
        <div>
          ...
          <h1>Mantenimiento de Gerencias</h1>
          <p className="subtitle">Gestión de áreas directivas y niveles de mando organizacional.</p>
        </div>
        <button 
                  ...
                  <tr key={g.id}>
                    <td>
                      <div className="flex items-center gap-3">
                        <div className="icon-box"><Building size={16} /></div>
                        <span className="font-bold">{g.name}</span>
                      </div>
                    </td>
                    <td><span className="org-pill">{g.organization.commercialName}</span></td>
                    <td>
                      <div className="flex gap-2">
                        <button onClick={() => { setFormData({ name: g.name, organizationId: g.organizationId }); setIsEditing(g.id); setShowForm(true); }} className="icon-btn"><Edit2 size={16} /></button>
        ...
        {showForm && (
          <div className="form-container card glass animate-slide-in-right">
            <div className="card-header border-b p-4 flex justify-between items-center">
              <h3 className="font-black">Configurar Gerencia</h3>
      ...
      <style jsx>{`
        ...
        .org-pill { background: #e0f2fe; color: #0369a1; padding: 0.25rem 0.75rem; border-radius: 999px; font-weight: 700; font-size: 0.75rem; }
