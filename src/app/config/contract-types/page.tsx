...
import { Plus, Trash2, Edit2, Save, X, FileText, ChevronRight, Settings, ToggleLeft, ToggleRight } from "lucide-react";
...
export default function ContractTypesPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/config/contract-types");
  ...
  const handleSave = async () => {
    ...
    const url = editingId ? `/api/config/contract-types/${editingId}` : "/api/config/contract-types";
    try {
  ...
  const handleDelete = async (id: string) => {
    ...
    const res = await fetch(`/api/config/contract-types/${id}`, { method: "DELETE" });
    if (!res.ok) {
      const data = await res.json();
    ...
    <Shell>
      <div className="page-header animate-fade-in">
        <div>
          ...
          <p className="subtitle">Gestión del catálogo de modalidades contractuales para empleados.</p>
        </div>
            ...
            <table className="custom-table">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={3} className="text-center py-8 text-slate-400">Sin registros. Crea el primero.</td></tr>
                ) : items.map(item => (
                  <tr key={item.id}>
                    <td>
        ...
        {showForm && (
          <div className="form-container card glass animate-slide-in-right">
            <div className="card-header border-b p-4 flex justify-between items-center">
              <h3 className="font-black">{editingId ? "Editar Tipo" : "Nuevo Tipo de Contrato"}</h3>
            ...
            <div className="p-6">
              ...
              <div className="form-group mb-4">
                ...
                <input
                  ...
                  placeholder="Ej: Indefinido, Plazo Fijo, Obra"
                />
              </div>
      ...
      <style jsx>{`
        ...
        .icon-btn:hover { background: #f1f5f9; color: var(--foreground); }
